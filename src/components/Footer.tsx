"use client";

import { motion } from "framer-motion";
import { Mail, ArrowUpRight } from "lucide-react";

const EMAIL = "joaquinrojas.content@gmail.com";
const WHATSAPP = "+56 9 9171 7490";
const WHATSAPP_URL = "https://wa.me/56991717490";

/* lucide no trae logos de marcas; el glifo oficial de WhatsApp va inline. */
function WhatsAppIcon({ size = 15 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
    </svg>
  );
}

export default function Footer() {
  return (
    <footer id="contacto" className="paper border-t border-[#16111F]/[0.07]">
      <div className="max-w-[1180px] mx-auto px-5 md:px-10 pt-20 pb-12">
        <div className="grid lg:grid-cols-[200px_1fr] gap-8 lg:gap-16">
          <span className="text-[11px] tracking-[0.28em] uppercase text-[#5C5468]">
            Contacto
          </span>

          <div className="max-w-[62ch]">
            <motion.h2
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.65, ease: [0.22, 1, 0.36, 1] }}
              className="text-3xl md:text-[42px] font-bold tracking-[-0.02em] leading-[1.12] text-[#16111F]"
            >
              ¡Hablemos!
            </motion.h2>

            <motion.p
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.65, delay: 0.08 }}
              className="mt-3 text-lg text-[#5C5468]"
            >
              Soy buena onda 😁
            </motion.p>

            {/* Contacto directo: WhatsApp y email, cada uno con su icono.
                Sin botón de copiar: el link de mailto ya alcanza. */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.65, delay: 0.16 }}
              className="mt-8 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3"
            >
              <a
                href={WHATSAPP_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="group inline-flex items-center justify-center gap-2.5 px-7 py-3.5 rounded-full bg-[#16111F] text-white text-sm font-semibold tracking-[0.02em] hover:bg-[#9B5CE5] transition-colors duration-300"
              >
                <WhatsAppIcon size={16} />
                {WHATSAPP}
                <ArrowUpRight
                  size={15}
                  className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-200"
                />
              </a>

              <a
                href={`mailto:${EMAIL}`}
                className="inline-flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-full border border-[#16111F]/15 text-[#5C5468] hover:text-[#16111F] hover:border-[#16111F]/35 text-sm font-medium transition-colors duration-300"
              >
                <Mail size={15} />
                {EMAIL}
              </a>
            </motion.div>
          </div>
        </div>

        {/* Barra inferior: solo el cierre, sin © ni cargo (pedido de Joaquín) */}
        <div className="mt-16 pt-8 border-t border-[#16111F]/[0.09] flex justify-center">
          <p className="text-lg font-bold tracking-[-0.01em] text-[#16111F]">
            ¡A darle con todo!
          </p>
        </div>
      </div>
    </footer>
  );
}
